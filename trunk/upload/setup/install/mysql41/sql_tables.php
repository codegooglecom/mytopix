...
// MySQL 4.1 does not support "DROP TABLE IF EXISTS":

$exists = array();
$sql    = $DB->query('SHOW TABLES');

while($row = $sql->getRow('row'))
{
    $exists[$row[0]] = true;
}

function myDropTable($table)
{
    global $exists;

    if(isset($exists[DB_PREFIX . $table]))
    {
        return 'DROP TABLE ' . DB_PREFIX . $table;
    }
}

// Thanks goes out to Peter Hall for creating such
// a clever work around for installation on various
// MySQL versions! 
// 
// ( code was modified from original version )


if(sizeof($exists))
{
    $query[] = myDropTable('active');
    $query[] = myDropTable('cache');
    $query[] = myDropTable('class');
    $query[] = myDropTable('emails');
    $query[] = myDropTable('emoticons');
    $query[] = myDropTable('events');
    $query[] = myDropTable('filter');
    $query[] = myDropTable('forums');
    $query[] = myDropTable('help');
    $query[] = myDropTable('macros');
    $query[] = myDropTable('members');
    $query[] = myDropTable('moderators');
    $query[] = myDropTable('notes');
    $query[] = myDropTable('polls');
    $query[] = myDropTable('posts');
    $query[] = myDropTable('skins');
    $query[] = myDropTable('templates');
    $query[] = myDropTable('titles');
    $query[] = myDropTable('topics');
    $query[] = myDropTable('tracker');
    $query[] = myDropTable('uploads');
    $query[] = myDropTable('vkeys');
    $query[] = myDropTable('voters');
}
