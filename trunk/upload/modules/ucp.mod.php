...
class ModuleObject extends MasterObject
{
    ...
    */
    function _main()
	{
        ...
        $per_day = $total_posts > 0
                 ? round($total_posts / $time_offset, 2)
                 : $this->LanguageHandler->blank;


		$per_day = $per_day > $total_posts 
                 ? $total_posts 
                 : $per_day;

        $this->LanguageHandler->main_posts_stat = sprintf($this->LanguageHandler->main_posts_stat,
                                                  $overall_posts . '%', $per_day);
        ...
        SELECT COUNT(notes_id) as Count 
        ...
        $this->LanguageHandler->main_notes_stat = sprintf($this->LanguageHandler->main_notes_stat,
                                                  $notes_full, $notes_left);
    ...
    */
    function _updateGeneral()
    {
        ...
        $location = $this->ParseHandler->parseText($location, F_CURSE);
        $homepage = $this->ParseHandler->parseText($homepage, F_CURSE);
        $aim      = $this->ParseHandler->parseText($aim,      F_CURSE);
        $icq      = $this->ParseHandler->parseText($icq,      F_CURSE);
        $yim      = $this->ParseHandler->parseText($yim,      F_CURSE);
        $msn      = $this->ParseHandler->parseText($msn,      F_CURSE);
    ...
    */
    function _subscriptions()
    {
        ...
        WHERE 
            ...
            tr.track_forum = 0
        ORDER BY tr.track_sent, tr.track_id", 
        __FILE__, __LINE__);
        ...
        WHERE 
            tr.track_user  = " . USER_ID . " AND
            tr.track_topic = 0
        ORDER BY tr.track_sent, tr.track_id", __FILE__, __LINE__);
    ...
    */
    function _avatar()
    {
        ...
        foreach($this->_AvatarHandler->getGalleryList() as $key => $val)
        {
        ...
        $avatar = $this->_AvatarHandler->fetchUserAvatar($this->UserHandler->getField('members_avatar_location'), 
                                                         $this->UserHandler->getField('members_avatar_dims'), true);
    ...
    */
    function _gallery()
    {
        ...
        $avatar_rows = '';
        $columns     = 0;
        foreach($this->_AvatarHandler->getGalleryAvatars($gallery) as $key => $val)
        {
            if(false == $columns)
            {
                $avatar_rows .= '<tr>';
        ...
        $avatar = $this->_AvatarHandler->fetchUserAvatar($this->UserHandler->getField('members_avatar_location'), false, true);
    ...
    */
    function _doGallery()
    {
        ...
        if(false == $this->_AvatarHandler->addGalleryAvatar($gallery, $avatar))
        {
            return $this->messenger(array('MSG' => $this->_AvatarHandler->getError()));
        }
    ...
    */
    function _updateAvatar()
    {
        ...
        if(false == $this->_AvatarHandler->addRemoteAvatar($this->post, $this->files, $this->UserHandler->getField('class_upload_avatars')))
        {
            return $this->messenger(array('MSG' => $this->_AvatarHandler->getError()));
        }
