...
class ModuleObject extends MasterObject
{
    ...
    */
	function _doTopicEdit()
	{
        ...
        if(isset($poll))
        {
            if(isset($remove))
            {
                ...
                foreach($choice as $key => $val)
                {
                    if($val)
                    {
                        $new_choices[]  = array('id' => $key, 'choice' => $val, 'votes' => (int) $votes[$key]);
                ...
                UPDATE " . DB_PREFIX . "polls SET 
                    poll_question   = '" . ucwords(strtolower($poll_title)) . "', 
		...
		$this->DatabaseHandler->query("
		UPDATE " . DB_PREFIX . "topics SET 
			topics_title   = '" . ucwords(strtolower($title)) . "', 
    ...
    */
    function _hideTopic()
    {
        ...
        if(false == $this->ForumHandler->getModAccess($row['topics_forum'], 'hide_topics'))
        {
            return $this->messenger(array('MSG' => 'err_no_perm'));
        }
        ...
        return $this->messenger(array('MSG' => 'mod_err_private_on', 'LINK' => "?gettopic={$this->_id}", 'LEVEL' => 1));
    }
    ...
    */
    function _showTopic()
    {
        ...
        return $this->messenger(array('MSG' => 'mod_err_private_off', 'LINK' => "?gettopic={$this->_id}", 'LEVEL' => 1));
    }
