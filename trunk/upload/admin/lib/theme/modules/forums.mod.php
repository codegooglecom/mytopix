...
class ModuleObject extends MasterObject
{
    ...
    */
	function execute()
	{
		...
		{
			case '00':
                $this->MyPanel->_make_nav(2, 7);

                if(false == $this->MyPanel->canAccess('admin_forum_add'))
                {
			...
			case '03':
                ...
                if(false == $this->MyPanel->canAccess('admin_forum_edit'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '06':
                ...
                if(false == $this->MyPanel->canAccess('admin_forum_remove'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
    ...
    */
	function _showAddForum()
	{
		...
		$this->MyPanel->form->startForm(GATEWAY . '?a=forums&amp;code=01');
			...
			$this->MyPanel->form->addCheckBox('post_approve', 1, false, false,
								            false, false, $this->LanguageHandler->forum_post_approve, false);

			$this->MyPanel->form->addCheckBox('topic_approve', 1, false, false,
								            false, false, $this->LanguageHandler->forum_topic_approve, false);

			$this->MyPanel->form->addCheckBox('post_count', 1, false, false,
								            false, true, $this->LanguageHandler->forum_post_counting, false);

			$this->MyPanel->form->addCheckBox('list_only', 1, false, false,
								            false, false, $this->LanguageHandler->forum_new_list_only_tip, false);
            ...
            while($row = $sql->getRow())
            {
				...
				$this->MyPanel->table->addRow(array(array($row['class_prefix'] . $row['class_title'] . $row['class_suffix'], false, 'headerb'),
										       array($this->MyPanel->form->addCheckBox('can_view_'   . $row['class_id'], 1, " checked=\"checked\"", false, true, false, false, 'center', 'checkwrap_1')),
                    					       array($this->MyPanel->form->addCheckBox('can_read_'   . $row['class_id'], 1, " checked=\"checked\"", false, true, false, false, 'center', 'checkwrap_2')),
                    						   array($this->MyPanel->form->addCheckBox('can_reply_'  . $row['class_id'], 1, " checked=\"checked\"", false, true, false, false, 'center', 'checkwrap_3')),
                    						   array($this->MyPanel->form->addCheckBox('can_start_'  . $row['class_id'], 1, " checked=\"checked\"", false, true, false, false, 'center', 'checkwrap_4')),
                    						   array($this->MyPanel->form->addCheckBox('can_upload_' . $row['class_id'], 1, " checked=\"checked\"", false, true, false, false, 'center', 'checkwrap'))));
    ...
    */
    function _doAddForum()
    {
        ...
        $this->MyPanel->messenger($this->LanguageHandler->forum_new_err_done, GATEWAY . '?a=forums&code=02');
    }
        ...
        if(false == $this->_forum)
        {
            foreach($list as $category)
            {
                if(false == $category['forum_parent'])
                {
                    ...
                    if($children = $this->ForumHandler->hasChildren($category['forum_id']))
                    {
                        $kids = true;
                    ...
                    if($kids)
                    {
                        ...
                        $delete = " <a href=\"" . GATEWAY . "?a=forums&amp;code=06&amp;forum={$category['forum_id']}\" onclick=\"javascript: return confirm('{$this->LanguageHandler->forum_list_js_delete}');\"><strong>{$this->LanguageHandler->link_delete}</strong></a>";
                    }
                    ...
                    $this->MyPanel->table->addRow(array(array("<strong><a href=\"" . GATEWAY . "?a=forums&amp;code=02&amp;forum={$category['forum_id']}\">" . $category['forum_name'] . '</a></strong>', ' style="background-color: #F7F7F7;"'),
                                                   ...
                                                   array("<a href=\"" . GATEWAY . "?a=forums&amp;code=04&amp;forum={$category['forum_id']}\" title=\"\">{$this->LanguageHandler->link_edit}</a> {$delete}", ' align="center" style="background-color: #F7F7F7;"', ' align="center"')));
                    ...
                    foreach($list as $forum)
                    {
                        if($forum['forum_parent'] == $category['forum_id'])
                        {
                            ...
                            if($kids)
                            {
                                ...
                                $delete = " <a href=\"" . GATEWAY . "?a=forums&amp;code=06&amp;forum={$forum['forum_id']}\" onclick=\"javascript: return confirm('{$this->LanguageHandler->forum_list_js_delete}');\"><strong>{$this->LanguageHandler->link_delete}</strong></a>";
                            }
                            ...
                            $this->MyPanel->table->addRow(array(array('|-- ' . $forum['forum_name'], false, 'headerb'),
                                                           ...
                                                           array("<a href=\"" . GATEWAY . "?a=forums&amp;code=04&amp;forum={$forum['forum_id']}\" title=\"\">{$this->LanguageHandler->link_edit}</a> {$delete}", ' align="center"')));
                        }
                    }
                }
            }
            ...
            if($children = $this->ForumHandler->hasChildren($this->_forum))
            {
                $kids = true;
            ...
            $this->MyPanel->table->addRow(array(array("<strong><a href=\"" . GATEWAY . "?a=forums&amp;code=02&amp;forum={$category['forum_parent']}\">Level Up</a> | " . $category['forum_name'] . '</strong>', ' style="background-color: #F7F7F7;"'),
                                           array('&nbsp;', ' style="background-color: #F7F7F7;"'),
                                           array($this->MyPanel->form->addSelect("pos[{$category['forum_id']}]", $positions, $category['forum_position'], false, false, true), " align='center' style='background-color: #F7F7F7;'" , 'headerb'),
                                           array("<a href=\"" . GATEWAY . "?a=forums&amp;code=04&amp;forum={$category['forum_id']}\" title=\"\">{$this->LanguageHandler->link_edit}</a> {$delete}", ' align="center" style="background-color: #F7F7F7;"')));
    ...
    */
    function _showEditForm()
    {
		...
		$this->MyPanel->appendBuffer($this->MyPanel->form->flushBuffer());
            ...
            $this->MyPanel->form->addTextArea('forum_description',$forum['forum_description'], 
                                             false, array(1,
                                             $this->LanguageHandler->forum_new_description_title,
                                             $this->LanguageHandler->forum_new_description_desc));
			...
			$this->MyPanel->form->addCheckBox('post_approve', 1, false, false,
								            false, $forum['forum_approve_topics'], $this->LanguageHandler->forum_post_approve, false);

			$this->MyPanel->form->addCheckBox('topic_approve', 1, false, false,
								            false, $forum['forum_approve_posts'], $this->LanguageHandler->forum_topic_approve, false);

			$this->MyPanel->form->addCheckBox('post_count', 1, false, false,
								            false, $forum['forum_enable_post_counts'], $this->LanguageHandler->forum_post_counting, false);

			$this->MyPanel->form->addCheckBox('list_only', 1, false, false,
								            false, $forum['forum_list_only'], $this->LanguageHandler->forum_new_list_only_tip, false);
    ...
    */
    function _doEditForum()
    {
        ...
        UPDATE " . DB_PREFIX . "forums SET
            ...
            forum_list_only          = " . (int) $list_only     . ",
            forum_approve_topics     = " . (int) $topic_approve    . ",
            forum_approve_posts      = " . (int) $post_approve     . "
