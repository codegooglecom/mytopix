...
class ModuleObject extends MasterObject
{
    ...
    */
	function execute()
	{
        ...
        if(false == $this->MyPanel->canAccess('admin_prune_topics'))
        {
		...
		$this->MyPanel->appendBuffer("<form method=\"post\" action=\"" . GATEWAY . '?a=prune&amp;code=02">');
            ...
            while($row = $sql->getRow())
            {
                ...
                $this->MyPanel->table->addRow(array(array("<strong>{$row['topics_id']}</strong>", false, 'headera'),
                                           ...
                                           array($this->MyPanel->form->addCheckBox('prune[]', $row['topics_id'], " checked=\"checked\"", false, true, false, false, false, 'c'), " valign=\"top\" align=\"center\"")));
            }
