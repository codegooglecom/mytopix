...
class FormHandler
{
    ...
    */
    function _getModeratorBit($announce = null, $locked = '', $pinned = '', $hidden = '')
    {
        ...
        if($this->_ForumHandler->getModAccess($this->_forum, 'hide_topics'))
        {
            $check['hidden'] = $hidden ? 'checked="checked"' : '';
            $hide = eval($this->_TemplateHandler->fetchTemplate('mod_option_hide'));
        }
        else {
            $hide = '';
        }
