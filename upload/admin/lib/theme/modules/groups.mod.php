...
class ModuleObject extends MasterObject
{
	...
	function execute()
	{
		...
		switch($this->_code)
		{
			...
			case '01':
                ...
                if(false == $this->MyPanel->canAccess('admin_group_add'))
                {
			...
			case '03':
                ...
                if(false == $this->MyPanel->canAccess('admin_group_edit'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '05':
                ...
                if(false == $this->MyPanel->canAccess('admin_group_remove'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '06':
                ...
                if(false == $this->MyPanel->canAccess('admin_group_masks'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
        ...
        while($row = $sql->getRow())
        {
            ...
            if(in_array($row['class_id'], array(1, 2, 3, 4, 5)))
            {
                ...
                $delete = " <a href=\"" . GATEWAY . "?a=groups&amp;code=05&amp;id={$row['class_id']}\"" .
                          ...
                          "{$this->LanguageHandler->link_delete}</a>";
            }
    ...
    */
    function _doAddGroup()
    {
        ...
        UPDATE " . DB_PREFIX . "class SET
            ...
            class_can_see_ghosts      = {$row['class_can_see_ghosts']},
            class_can_see_anything    = {$row['class_can_see_anything']},
            class_can_bypass_simple   = {$row['class_can_bypass_simple']}
		WHERE class_id = {$id}");
    ...
    function _editGroup()
    {
		...
		$this->MyPanel->appendBuffer($this->MyPanel->form->flushBuffer());
			...
			$this->MyPanel->table->startTable($this->LanguageHandler->group_tbl_perm_header);
				...
				$this->MyPanel->table->addRow(array(array($this->LanguageHandler->group_tbl_see_anything, false, 'headerb'),
										   array($this->MyPanel->form->addYesNo('class_can_see_anything', 1, false, false, true, 
										   ($row['class_can_see_anything'] ? true : false)), " width='25%'", 'headerc')));
				...
				$this->MyPanel->table->addRow(array(array($this->LanguageHandler->group_tbl_see_ghosts, false, 'headerb'),
										   array($this->MyPanel->form->addYesNo('class_can_see_ghosts', 1, false, false, true, 
										   ($row['class_can_see_ghosts'] ? true : false)), " width='20%'", 'headerc')));

				$this->MyPanel->table->addRow(array(array($this->LanguageHandler->group_tbl_bypass, false, 'headerb'),
										   array($this->MyPanel->form->addYesNo('class_can_bypass_simple', 1, false, false, true, 
										   ($row['class_can_bypass_simple'] ? true : false)), " width='20%'", 'headerc')));
    ...
    */
    function _doEditGroup()
    {
        ...
        UPDATE " . DB_PREFIX . "class SET
            class_title              = '{$title}',
            class_prefix             = '" . $this->ParseHandler->uncleanString($class_prefix) . "',
            class_suffix             = '" . $this->ParseHandler->uncleanString($class_suffix) . "',
            ...
            class_can_see_ghosts     = " . (int) $class_can_see_ghosts      . ",
            class_can_see_anything   = " . (int) $class_can_see_anything    . ",
			class_can_bypass_simple  = " . (int) $class_can_bypass_simple   . "
