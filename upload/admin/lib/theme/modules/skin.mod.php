...
class ModuleObject extends MasterObject
{
	...
	function execute()
	{
		...
		switch($this->_code)
		{
			...
			case '01':
                ...
                if(false == $this->MyPanel->canAccess('admin_skin_edit'))
                {
			...
			case '03':
                ...
                if(false == $this->MyPanel->canAccess('admin_skin_delete'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '04':
                ...
                if(false == $this->MyPanel->canAccess('admin_skin_add'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '06':
                ...
                if(false == $this->MyPanel->canAccess('admin_skin_install'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '08':
                ...
                if(false == $this->MyPanel->canAccess('admin_skin_export'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '10':
                ...
                if($this->MyPanel->canAccess('admin_skin_export')  ||
                   $this->MyPanel->canAccess('admin_skin_add')     ||
                   $this->MyPanel->canAccess('admin_skin_delete')  ||
                   $this->MyPanel->canAccess('admin_skin_install') ||
                   $this->MyPanel->canAccess('admin_skin_edit'))
                {
    				$this->_doRemFile();
                }
                else {
    ...
    */
	function _doUpload()
	{
        ...
        $UploadHandler->setMaxSize(999999);
    ...
    */
	function _doExport()
	{
		...
		while($row = $sql->getRow())
		{
			...
			$export_data .= "\$temps[] = \"INSERT INTO {\$this->config['db_pref']}templates VALUES ({\$dir}, $values);\";\r\n";
		}
		...
		while($row = $sql->getRow())
		{
			...
			$export_data .= "\$emots[] = \"INSERT INTO {\$this->config['db_pref']}emoticons VALUES (null, {\$dir}, $values);\";\r\n";
		}
		...
		while($row = $sql->getRow())
		{
			...
			$export_data .= "\$titles[] = \"INSERT INTO {\$this->config['db_pref']}titles VALUES (null, $values, {\$dir});\";\r\n";
		}
		...
		while($row = $sql->getRow())
		{
			...
			$export_data .= "\$emots[] = \"INSERT INTO {\$this->config['db_pref']}macros VALUES (null, {\$dir}, $values);\";\r\n";
		}
