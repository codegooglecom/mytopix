	...
	function execute()
	{
		...
		switch($this->_code)
		{
			...
			case '01':
                ...
                if(false == $this->MyPanel->canAccess('admin_word_edit'))
                {
			...
			case '04':
                ...
                if(false == $this->MyPanel->canAccess('admin_word_remove'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '05':
                ...
                if(false == $this->MyPanel->canAccess('admin_word_create'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '07':
                ...
                if(false == $this->MyPanel->canAccess('admin_word_import'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '09':
                ...
                if(false == $this->MyPanel->canAccess('admin_word_export'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
			...
			case '11':
                ...
                if($this->MyPanel->canAccess('admin_word_edit')   ||
                   $this->MyPanel->canAccess('admin_word_create') ||
                   $this->MyPanel->canAccess('admin_word_import') ||
                   $this->MyPanel->canAccess('admin_word_export') ||
                   $this->MyPanel->canAccess('admin_word_remove'))
                {
			...
			$this->MyPanel->table->startTable(sprintf($this->LanguageHandler->word_wtbl_header, $this->LanguageHandler->$title));
				...
				foreach($lang as $key => $val)
				{
					...
					$this->MyPanel->table->addRow(array(array($this->MyPanel->form->addTextBox('name[]',  $key, false, false, true) . $var, " valign='top'"),
											                   $this->MyPanel->form->addTextArea('value[]', $this->ParseHandler->uncleanString(stripslashes($val)), " style=\"width: 90%;\"", false, true)));
				}
    ...
    */
	function _doEditWords()
	{
		...
		for($i = 0; $i < count($name); $i++)
		{
			if($name[$i] || $value[$i])
            {
                $mod[$name[$i]] = $this->ParseHandler->uncleanString($value[$i], true);
            }
		}
            ...
            foreach($packs as $language)
			{
				$delete = $language == 'english' 
                        ...
                        : "<a href=\"" . GATEWAY . "?a=words&amp;code=04&amp;language={$language}\" onclick='javascript:return confirm" . 
                          "(\"{$this->LanguageHandler->word_pack_err_confirm}\");'>{$this->LanguageHandler->link_delete}</a>" ;
                ...
                $this->MyPanel->table->addRow(array(array(ucwords($language), false, 'headerb'),
                                               ...
                                               array($delete, " align='center'", 'headerc')));
			}
