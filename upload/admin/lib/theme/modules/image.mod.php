    ...
    function execute()
	{
		...
		switch($this->_code)
		{
			...
			case '01':
                ...
                if(false == $this->MyPanel->canAccess('admin_images_add'))
                {
			...
			case '03':
                ...
                if(false == $this->MyPanel->canAccess('admin_images_remove'))
                {
                    $this->MyPanel->warning($this->LanguageHandler->role_no_perm, false);
                }
                else {
    ...
    */
    function _doDeleteImage()
    {
        ...
        if(is_writable($dir))
        {
            ...
            $ext = end(explode('.', $this->_file));

            if(false == in_array($ext, $this->config['good_image_types']))
            {
                $this->MyPanel->messenger($this->LanguageHandler->image_err_not_found, GATEWAY . "?a=image&amp;skin={$this->_skin}");
            }

            if(false == file_exists($dir . $this->_file))
            {
                $this->MyPanel->messenger($this->LanguageHandler->image_err_not_found, GATEWAY . "?a=image&amp;skin={$this->_skin}");
            }
