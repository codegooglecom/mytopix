...
class ForumHandler
{
    ...
    */
    function getModSelect($forum, $topic)
    {
        ...
        if($this->getModAccess($forum, 'hide_topics'))
        {
            if($topic['topics_hidden'])
            {
                $out .= "<option value=\"11\">{$this->_LanguageHandler->mod_drop_public}</option>\n";
            }
            else {
                $out .= "<option value=\"10\">{$this->_LanguageHandler->mod_drop_private}</option>\n";
            }
        }
