...
class ParseHandler
{
    ...
    */
	function parseSimple($string)
	{
        ...
        while(preg_match("#\[face=(.*?)\](.*?)\[/face\]#si", $string))
        {
            $string = preg_replace("#\[face=(.*?)\](.*?)\[/face\]#si", "<span style=\"font-family: $1;\">$2</span>", $string);
        }

        while(preg_match("#\[size=([^\]]+)\](.+?)\[/size\]#ies", $string))
        {
            $string = preg_replace("#\[size=(.*?)\](.*?)\[/size\]#si", "<span style=\"font-size: $1px;\">$2</span>", $string);
        }
